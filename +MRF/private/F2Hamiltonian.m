function [ H ] = F2Hamiltonian( t, Zs, RFs, Rabi, theta )
...
cp    = sum( c .* ( alpha * e.^(1i .* t .* 2 .* pi .* RFs ) + beta * e.^(-1i .* t .* 2 .* pi .* RFs ) ), 1) / (2.^0.5);
cn    = sum( c .* ( alpha * e.^(-1i .* t .* 2 .* pi .* RFs ) + beta * e.^(1i .* t .* 2 .* pi .* RFs ) ), 1) / (2.^0.5);
