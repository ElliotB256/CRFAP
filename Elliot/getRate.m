function totalRate = getRate(RF, RFp, gRF, gRFp)
...
%H0 = kron(I_RFp, kron(I_RF, H_a)) + kron(I_RFp, kron(H_RF, I_a)) + kron(H_RFp, kron(I_RF, I_a));
...
ii = find(all(states == repmat([ 1 round(N/2) 1 ], size(states,1), 1), 2));
initial = vec; initial(round(N/Np/2)) = 1; initial = E * initial;
